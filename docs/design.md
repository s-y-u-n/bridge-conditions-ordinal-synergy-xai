...
原則として **ゲームテーブル `value` は「大きいほど良い」スコア**とする（例: `inv_mae=1/(1+MAE)`）。ただし回帰で `metric=mae` を使う場合は例外として **小さいほど良い**。

### 8.4 空集合（無情報）ベースライン `v(∅)`

比較の都合上、「特徴量なし（無情報）」での理論的スコア `v(∅)` を別途計算できるようにする（ゲームテーブルとは別CSV）。

- 分類（`accuracy`）: `y_train` の最頻クラスを常に予測
- 回帰（`neg_mae` / `inv_mae`）: `y_train` の中央値を常に予測（MAE最適）
- 分割はゲームテーブルと同じ（`test_size`, `seed`; 分類は可能なら stratify）

既定の出力先:

- `outputs/<dataset_key>/game_tables/empty_baseline.csv`

### 8.5 スコアの弱順序（同値類つきランキング）

ゲームテーブルの単発スコア `v(S)` をそのまま使うのではなく、**連続区間制約つきの最適1次元区間分割**により
弱順序（同値類つきランキング）`Σ1 ≻ Σ2 ≻ … ≻ Σk` を自動生成できるようにする。

- 入力: ゲームテーブル（少なくとも `value`/`abs_value` 等のスコア列を持つCSV）
- 制約: `score` を降順ソートした列 `x[1..n]` 上で、各クラス `Σr` は**連続区間**でなければならない
- コスト: 各区間は区間内平均との差の二乗和（SSE）
- 目的: `k` 分割の総SSEを厳密最小化（近似不可）
- `k` 選択: `k=1..k_max` の最適SSEを計算し、BIC/MDL（同式で可）により最良 `k` を選ぶ

用途:

- スコアの“区切り”をデータ駆動で決めて、rank（クラスID）をゲームテーブルに付与する
- プロット上にも分割線（境界）を重ねて確認できる

### 8.6 提携×特徴量のヒートマップ可視化

ゲームテーブルの 0/1 指示変数（特徴量を使ったかどうか）を、スコアの良い順に並べた提携（coalition）ごとに可視化する。

- 行（縦軸）: coalition（スコアが良い順に並べる）
  - 分類（`accuracy` など）: スコア大きい順
  - 回帰（`mae`）: スコア小さい順
- 列（横軸）: 特徴量（0/1 指示変数列）
- セル: その coalition が当該特徴量を使っていれば True（1）として着色（赤）

用途:

- 上位（あるいは下位）スコアの coalition がどの特徴量を含みやすいかを直感的に確認
- ランキング境界（class_id）と組み合わせて、境界付近で特徴量集合がどう変わるかを見る

実装は `outputs/**/game_tables/*.csv`（ワイド形式）を入力とし、必要なら上位 `top_n` のみを描画して視認性を担保する。
